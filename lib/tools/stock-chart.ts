...
export const stockChartTool = tool({
  ...
  execute: async ({
    ...
    // Telegram mini app unlocks premium data by default
    const isProUser = true;
    const actualIncludeEarnings = true;
    const actualIncludeStatistics = Boolean(include_statistics);
    const actualIncludeBalanceSheet = Boolean(include_balance_sheet);
    const actualIncludeIncomeStatement = Boolean(include_income_statement);
    const actualIncludeCashFlow = Boolean(include_cash_flow);
    const actualIncludeDividends = Boolean(include_dividends);
    const actualIncludeInsiderTransactions = Boolean(include_insider_transactions);
    const actualIncludeMarketMovers = Boolean(include_market_movers);
    const actualFilingTypes = filing_types;
