...
const createSearchStrategy = (
  ...
  const strategies = {
    parallel: () => {
      if (!clients.parallel) {
        throw new Error('Parallel API key not configured. Set PARALLEL_API_KEY or choose a different search provider.');
      }
...
export function webSearchTool(
  ...
  return tool({
    ...
    execute: async ({
      ...
      const parallelApiKey = serverEnv.PARALLEL_API_KEY;
      const clients = {
        exa: new Exa(serverEnv.EXA_API_KEY),
        parallel:
          parallelApiKey && parallelApiKey !== 'dummy-parallel-api-key'
            ? new Parallel({ apiKey: parallelApiKey })
            : null,
