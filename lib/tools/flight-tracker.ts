...
export const flightTrackerTool = tool({
  ...
  execute: async ({
    ...
    const { AMADEUS_API_KEY, AMADEUS_API_SECRET } = serverEnv;
    const missingCredentials =
      !AMADEUS_API_KEY ||
      !AMADEUS_API_SECRET ||
      AMADEUS_API_KEY === 'dummy-amadeus-api-key' ||
      AMADEUS_API_SECRET === 'dummy-amadeus-api-secret';

    if (missingCredentials) {
      return {
        data: [],
        error: 'Amadeus API credentials are not configured. Please set AMADEUS_API_KEY and AMADEUS_API_SECRET.',
      };
    }
