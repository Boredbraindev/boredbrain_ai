...
const searchWeb = async (query: string, category?: SearchCategory, include_domains?: string[]) => {
  ...
  try {
    ...
    console.log(`searchWeb received ${response.results.length} results from Tavily API`);
    ...
    const mappedResults: SearchResult[] = response.results.map((r) => ({
      ...
      favicon: '', // Tavily doesn't provide favicon
    }));
...
const getContents = async (links: string[]) => {
  ...
  // Use Firecrawl for all URLs (no longer using Exa)
  console.log('Using Firecrawl for all URLs');
...
async function extremeSearch(
  ...
  const { object: result } = await generateObject({
    model: boredbrain.languageModel('boredbrain-grok-4-fast-think'),
    schema: z.object({
      plan: z
        .array(
          z.object({
