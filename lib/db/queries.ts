...
import { user, chat, type User, message, type Message, type Chat, stream, extremeSearchUsage, messageUsage, customInstructions, lookout } from './schema';
...
// Payment helpers (stubs for Telegram mini app)
export async function getPaymentsByUserId(_: { userId: string }) {
  return [];
}

export async function getPaymentById(_: { paymentId: string }) {
  return null;
}

export async function getSuccessfulPaymentsByUserId(_: { userId: string }) {
  return [];
}

export async function getTotalPaymentAmountByUserId(_: { userId: string }) {
  return 0;
}

export async function hasSuccessfulDodoPayment(_: { userId: string }) {
  return false;
}

export async function isDodoPaymentsProExpired(_: { userId: string }) {
  return true;
}

export async function getDodoPaymentsExpirationInfo(_: { userId: string }) {
  return null;
}
...
export async function getSignalsByUserId({ userId }: { userId: string }) {
  try {
...
export async function getSignalById({ id }: { id: string }) {
  try {
...
export async function updateSignalLastRun({
  id,
  lastRunAt,
  lastRunChatId,
  nextRunAt,
  ...
  try {
    // Get current signal to append to run history
    const currentSignal = await getSignalById({ id });
    if (!currentSignal) {
      throw new Error('Signal not found');
    }

    const currentHistory = (currentSignal.runHistory as any[]) || [];
    ...
    const [updatedSignal] = await db.update(lookout).set(updateData).where(eq(lookout.id, id)).returning();

    return updatedSignal;
  } catch (error) {
    throw new ChatSDKError('bad_request:database', 'Failed to update signal last run');
  }
}

// New function to get run statistics
export async function getSignalRunStats({ id }: { id: string }) {
  try {
    const signal = await getSignalById({ id });
    if (!signal) return null;

    const runHistory = (signal.runHistory as any[]) || [];
    ...
    console.error('Error getting signal run stats:', error);
    return null;
  }
}

export async function deleteSignal({ id }: { id: string }) {
  try {
    const [deletedSignal] = await db.delete(lookout).where(eq(lookout.id, id)).returning();

    return deletedSignal;
  } catch (error) {
    throw new ChatSDKError('bad_request:database', 'Failed to delete signal');
  }
}
