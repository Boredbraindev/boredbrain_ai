...
const Messages: React.FC<MessagesProps> = ({
  ...
  const handleSuggestedQuestionClick = useCallback(
    async (question: string) => {
      ...
      await sendMessage({
        parts: [{ type: 'text', text: question.trim() } as ChatMessage['parts'][number]],
        role: 'user',
      });
    },
  ...
  return (
    <div className="space-y-0 pb-32 sm:pb-40 flex flex-col px-3 sm:px-4">
      <div className="flex-grow max-w-4xl w-full mx-auto">
        {memoizedMessages.map((message, index) => {
          ...
          if (isCurrentMessageUser && isNextMessageAssistant) {
            // Tight spacing between user message and assistant response
            messageClasses = 'mb-4';
          } else if (isCurrentMessageAssistant && index < memoizedMessages.length - 1) {
            // Clear separation between conversation pairs
            messageClasses = 'mb-8 pb-8 border-b border-border/40 dark:border-border/40';
          } else if (isCurrentMessageAssistant && index === memoizedMessages.length - 1) {
            // Last message breathing room
            messageClasses = 'mb-2';
          } else {
            messageClasses = 'mb-2';
          }
