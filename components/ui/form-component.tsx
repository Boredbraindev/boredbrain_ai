...
import { CpuIcon, Crown02Icon, DocumentAttachmentIcon, ConnectIcon } from '@hugeicons/core-free-icons';
...
const ModelSwitcher: React.FC<ModelSwitcherProps> = React.memo(
  ({
    ...
    useEffect(() => {
      ...
      if (currentModelExists && currentModelRequiresPro && !isProUser && selectedModel !== 'boredbrain-default') {
              ...
              <CommandGroup key="best-matches">
                <div
                  className={cn(
                    ...
                    isMobile ? 'text-xs' : 'text-[11px] uppercase tracking-wider opacity-70',
                  )}
                >
                  Best matches
                </div>
                  ...
                  return (
                    <CommandItem
                      ...
                      className={cn(
                        ...
                        'transition-all duration-200',
                        'hover:bg-muted/50',
                        'data-[selected=true]:bg-gradient-to-r data-[selected=true]:from-[oklch(0.65_0.26_258)]/10 data-[selected=true]:to-[oklch(0.78_0.25_320)]/10',
                      )}
                    >
                      <div className="flex flex-col min-w-0 flex-1">
                        <div
          ...
          <Drawer open={open} onOpenChange={setOpen}>
            ...
            <DrawerContent className="min-h-[60vh] max-h-[80vh] flex flex-col">
              <DrawerHeader className="pb-4 flex-shrink-0">
                <DrawerTitle className="text-left flex items-center gap-2 font-medium font-sans text-lg">
          ...
          <DialogContent className="p-0 overflow-hidden gap-0 bg-background sm:max-w-[450px]" showCloseButton={false}>
                ...
                <div className="relative z-10 flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-2 min-w-0 flex-1">
                      {selectedProModel?.label ? (
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                          <span className="text-xl sm:text-2xl font-sans font-bold">Bored Brain</span>
                          <ProBadge className="!text-white !bg-white/20 !ring-white/30 font-extralight" />
                        </div>
                      )}
                    </div>
                  ...
                  <DialogDescription className="text-white/90">
                    <p className="text-sm text-white/80 text-left mt-2">
                      Premium subscriptions are on pause while the Telegram mini app is open access. Enjoy the advanced
                      models and limits on the house for now.
                    </p>
                  </DialogDescription>
                </div>
              </div>
            ...
            <div className="px-6 py-6 flex flex-col gap-4">
              ...
              <div className="flex items-center gap-4">
                <CheckIcon className="size-4 text-primary flex-shrink-0" />
                <div className="space-y-1">
                  <p className="text-sm font-medium text-foreground">Bored Brain Signals</p>
...
const GroupModeToggle: React.FC<GroupSelectorProps> = React.memo(
  ({ selectedGroup, onGroupSelect, status, onOpenSettings, isProUser }) => {
    ...
    const [searchProvider] = useLocalStorage<SearchProvider>('boredbrain-search-provider', 'firecrawl');
    ...
    return (
      <div className="flex items-center">
        ...
        <div className="flex items-center bg-muted/30 backdrop-blur-sm border border-border/40 rounded-lg !gap-1 !py-1 !px-0.75 h-8">
                  ...
                  <DrawerTrigger asChild>
                    <Button
                      ...
                      className={cn(
                        ...
                        !isExtreme
                          ? 'bg-gradient-to-br from-secondary/25 via-primary/20 to-accent/25 hover:from-secondary/30 hover:via-primary/25 hover:to-accent/30 dark:bg-gradient-to-br dark:from-primary dark:via-secondary dark:to-primary dark:hover:from-primary/90 dark:hover:via-secondary/90 dark:hover:to-primary/90 text-white dark:text-white ring-1 ring-ring/20 shadow-sm'
                          : 'text-muted-foreground hover:bg-muted/50',
                      )}
                    >
                      {selectedGroupData ? (
                        selectedGroupData.id === 'web' ? (
                          <TreePalm size={18} weight="duotone" className="text-primary" />
                        ) : (
                          <HugeiconsIcon icon={selectedGroupData.icon} size={30} color="currentColor" strokeWidth={2} />
                        )
                      ) : (
                        <TreePalm size={18} weight="duotone" className="text-primary/70" />
                      )}
                    </Button>
                  </DrawerTrigger>
                </TooltipTrigger>
                    ...
                    {visibleGroups.map((group) => (
                      <button
                        ...
                        className={cn(
                          ...
                          selectedGroup === group.id ? 'bg-muted/70 border-primary/30' : 'bg-background/50',
                        )}
                      >
            ...
            <TooltipTrigger asChild>
              <Button
                ...
                className={cn(
                  ...
                  isExtreme ? 'neon-extreme text-white font-semibold' : 'text-muted-foreground hover:bg-muted/50',
                )}
              >
                <Brain size={18} weight="duotone" className="transition-transform" />
              </Button>
            </TooltipTrigger>
            <TooltipContent side="bottom">
...
const FormComponent: React.FC<FormComponentProps> = ({
  ...
  // Toggle visibility of optional toolbar actions (design request)
  const showAttachmentButton = false;
  const showEnhanceButton = false;
  ...
  const handleEnhance = useCallback(async () => {
    if (!isProUser) {
      toast.info('Subscriptions are paused while the Telegram mini-app is free for everyone.');
      setShowUpgradeDialog(true);
      return;
    }
  ...
  }, [input, isProcessing, isProUser, setInput, inputRef, typewriterText, isEnhancing, setShowUpgradeDialog]);
    ...
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (blockedPdfFiles.length > 0) {
        ...
        toast.error(`PDF uploads require Pro subscription. Upgrade to access PDF analysis.`, {
          action: {
            label: 'Upgrade',
            onClick: () => toast.info('Subscriptions are paused while the Telegram mini-app is free for everyone.'),
          },
        });
      }
      ...
      <TooltipProvider>
        <div
          className={cn(
            ...
            attachments.length > 0 || uploadQueue.length > 0
              ? 'bg-muted/30 !backdrop-blur-sm p-1 shadow-sm shadow-black/5 dark:shadow-black/10'
              : 'bg-transparent',
          )}
        ...
        >
          <AnimatePresence>
            {isDragging && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="absolute inset-0 backdrop-blur-md bg-background/70 rounded-lg border border-dashed border-border/60 flex items-center justify-center z-50 m-2 shadow-xl shadow-black/10 dark:shadow-black/25"
              >
          ...
          <div className="relative">
            ...
            <div
              className={cn(
                'relative rounded-xl bg-muted/80 backdrop-blur-sm border border-border/40 focus-within:border-ring/50 transition-all duration-200',
                'border-0',
                (isEnhancing || isTypewriting) && 'bg-muted/90',
              )}
            >
              {isRecording ? (
                <Textarea
              ...
              >
                ...
                <div className={cn('flex items-center flex-shrink-0 gap-1')}>
                  {showAttachmentButton && hasVisionSupport(selectedModel) && (
                  ...
                  {showEnhanceButton && (input.length > 0 || isEnhancing || isTypewriting) && (
                    <Tooltip delayDuration={300}>
                      <TooltipTrigger asChild>
                        <Button
                          size="icon"
                  ...
                  {isProcessing ? (
                    <Tooltip delayDuration={300}>
                      <TooltipTrigger asChild>
                        <Button
                          variant="ghost"
                          size="icon"
                          className="shimmer-button group rounded-full transition-all duration-200 !size-8 bg-gradient-to-br from-secondary/25 via-primary/20 to-accent/25 hover:from-secondary/30 hover:via-primary/25 hover:to-accent/30 dark:bg-gradient-to-br dark:from-primary dark:via-secondary dark:to-primary dark:hover:from-primary/90 dark:hover:via-secondary/90 dark:hover:to-primary/90 ring-1 ring-ring/35 ring-offset-1 ring-offset-background shadow-lg"
                          onClick={(event) => {
                            event.preventDefault();
                            event.stopPropagation();
                            if (!isEnhancing && !isTypewriting) {
                    ...
                    /* Voice Recording Button - Currently Disabled */
                    /* Uncomment below to enable voice recording feature */
                    /*
                    <Tooltip delayDuration={300}>
                      <TooltipTrigger asChild>
                        <Button
                          ...
                          className={cn('group rounded-full m-auto transition-all duration-200 !size-8', !isRecording && 'bg-primary hover:bg-primary/90 text-primary-foreground')}
                          onClick={(event) => {
                            event.preventDefault();
                            event.stopPropagation();
                            if (!isEnhancing && !isTypewriting) {
                    ...
                    */
                    /* Show Send Button even when no input (disabled state) */
                    <Tooltip delayDuration={300}>
                      <TooltipTrigger asChild>
                        <Button
                          size="icon"
                          className="group rounded-full flex m-auto transition-all duration-200 !size-8 bg-gradient-to-br from-secondary/25 via-primary/20 to-accent/25 hover:from-secondary/30 hover:via-primary/25 hover:to-accent/30 dark:bg-gradient-to-br dark:from-primary dark:via-secondary dark:to-primary dark:hover:from-primary/90 dark:hover:via-secondary/90 dark:hover:to-primary/90 ring-1 ring-ring/35 ring-offset-1 ring-offset-background shadow-sm"
                          onClick={(event) => {
                            event.preventDefault();
                            event.stopPropagation();
                            if (!isEnhancing && !isTypewriting && input.length > 0) {
                              submitForm();
                            }
                          }}
                          disabled={true}
                      ...
                      <TooltipContent
                        ...
                        className="border-0 backdrop-blur-xs py-2 px-3 !shadow-none"
                      >
                        <span className="font-medium text-[11px]">Type a message to send</span>
                      </TooltipContent>
                    </Tooltip>
                  ) : (
            ...
            <DialogHeader className="p-2">
              <div className="relative w-full p-6 rounded-md text-white overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col gap-4">
                  ...
                  <DialogDescription className="text-white/90">
                    <p className="text-sm text-white/80 text-left">
                      Premium subscriptions are paused while we keep everything open access. Enhanced capabilities stay
                      available to everyone during this period.
                    </p>
                  </DialogDescription>
                  <Button
                    onClick={() => {
                  ...
                  >
                    {discountConfig?.buttonText || 'Subscriptions paused'}
                  </Button>
                </div>
              </div>
            </DialogHeader>
