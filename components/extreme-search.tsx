...
import { XLogoIcon, TreePalm } from '@phosphor-icons/react/dist/ssr';
      ...
      <div className="w-full">
        <div
          className={cn(
            ...
            'bg-white dark:bg-neutral-900 card-surface',
            'border border-neutral-200 dark:border-neutral-800',
  ...
  if (isCompleted) {
    ...
    return (
      <div className="space-y-2">
        {/* Research Process */}
        <Card className="overflow-hidden rounded-xl border border-border/40 bg-muted/15 backdrop-blur-sm shadow-none">
          <button
            type="button"
            className={cn(
              'flex w-full items-center justify-between px-4 py-3 transition-colors text-left',
              researchProcessOpen ? 'bg-muted/30' : 'hover:bg-muted/25',
            )}
            onClick={() => setResearchProcessOpen((prev) => !prev)}
          >
            <span className="flex items-center gap-2">
              <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-primary/15 text-primary">
                <TreePalm size={16} weight="duotone" />
              </span>
              <span className="text-sm font-semibold text-foreground tracking-tight">Research Process</span>
            </span>
            <span className="flex items-center gap-2 text-xs text-muted-foreground">
              {researchProcessOpen ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
            </span>
          </button>
          <AnimatePresence>
            {researchProcessOpen && (
              ...
              >
                <CardContent className="px-4 pb-4 !pt-0">
                  <div className="max-h-[280px] overflow-y-auto pr-1 text-sm scrollbar-thin scrollbar-thumb-border/60 hover:scrollbar-thumb-border/80 dark:scrollbar-thumb-border/50 dark:hover:scrollbar-thumb-border/70 scrollbar-track-transparent">
                    {renderTimeline()}
                  </div>
                </CardContent>
              </motion.div>
