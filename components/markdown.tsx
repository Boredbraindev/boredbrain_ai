...
const LazyCodeBlockComponent: React.FC<CodeBlockProps> = ({ children, language, elementKey }) => {
  ...
  return (
    <div className="group relative my-5 rounded-md border border-muted-foreground/20 bg-muted/30 backdrop-blur-sm overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2 bg-muted/50 border-b border-muted-foreground/20">
        <div className="flex items-center gap-2">
          {language && (
            <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider opacity-70">{language}</span>
          )}
          <span className="text-xs text-muted-foreground opacity-70">{lineCount} lines</span>
        </div>
