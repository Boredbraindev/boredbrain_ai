/**
 * GET /api/predict/feed — Returns recent prediction bets (live feed)
 * POST /api/predict/feed — Auto-generate prediction bets (called by heartbeat)
 */
...
// ─── In-memory prediction feed (persists across requests in serverless) ─────
...
function generateBet(roundId?: number): PredictBet {
  const isAgent = Math.random() > 0.35;
  const bettor = isAgent
    ? AGENT_BETTORS[Math.floor(Math.random() * AGENT_BETTORS.length)]
    : null;
  ...
  return {
    ...
    user: isAgent ? bettor!.name : USER_BETTORS[Math.floor(Math.random() * USER_BETTORS.length)],
...
if (predictFeed.length === 0) {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    bet.timestamp = now - (30 - i) * (Math.random() * 4000 + 2000);
    predictFeed.push(bet);
  }
  predictFeed.sort((a, b) => b.timestamp - a.timestamp);
}

// ─── GET: Return recent bets ─────────────────────────────────────────────────
...
export async function GET(request: NextRequest) {
  ...
  // Stats
  const upBets = predictFeed.filter(b => b.direction === 'UP');
  const downBets = predictFeed.filter(b => b.direction === 'DOWN');
  const upVolume = upBets.reduce((s, b) => s + b.amount, 0);
  const downVolume = downBets.reduce((s, b) => s + b.amount, 0);

  return apiSuccess({
    bets,
    stats: {
      totalBets: predictFeed.length,
      upCount: upBets.length,
      downCount: downBets.length,
      upVolume,
      downVolume,
...
// ─── POST: Generate new bets (called by heartbeat) ──────────────────────────
...
export async function POST(request: NextRequest) {
  ...
  const newBets: PredictBet[] = [];

  for (let i = 0; i < count; i++) {
    const bet = generateBet(roundId);
    // Stagger timestamps slightly
    bet.timestamp = Date.now() - i * (Math.random() * 500 + 100);
    newBets.push(bet);
    predictFeed.unshift(bet);
  }
