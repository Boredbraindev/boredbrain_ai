      ...
      const result = streamText({
        ...
        system:
          ...
          (customInstructions && (isCustomInstructionsEnabled ?? true)
            ? `\n\nThe user's custom instructions: ${(customInstructions.content || '').slice(0, 2000).replace(/```/g, '')}`
            : '') +
          (latitude && longitude ? `\n\nThe user's location is ${latitude}, ${longitude}.` : ''),
        toolChoice: 'auto',
        providerOptions: {
          openai: {
            ...(model !== 'boredbrain-qwen-coder'
              ? {
          ...
          groq: {
            ...(model === 'boredbrain-gpt-oss-20' || model === 'boredbrain-gpt-oss-120'
              ? {
                  reasoningEffort: 'medium',
                  reasoningFormat: 'hidden',
                }
              : {}),
            ...(model === 'boredbrain-qwen-32b'
              ? {
        ...
        tools: (() => {
          const baseTools = {
            ...
            nft_retrieval: nftRetrievalTool,
            token_retrieval: tokenRetrievalTool,
            wallet_analyzer: walletAnalyzerTool,
