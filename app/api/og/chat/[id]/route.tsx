...
async function loadAsset(relativePath: string) {
  ...
  if (!response.ok) {
    throw new Error(`Failed to load asset at ${relativePath}`);
  }
...
function arrayBufferToBase64(buffer: ArrayBuffer): string {
  ...
  for (; i + 2 < bytes.length; i += 3) {
    ...
    base64 += base64Chars[((bytes[i] & 0x03) << 4) | (bytes[i + 1] >> 4)];
    base64 += base64Chars[((bytes[i + 1] & 0x0f) << 2) | (bytes[i + 2] >> 6)];
...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [bgImageBuffer, interFontData, beVietnamProFontData] = await Promise.all([
      loadAsset('../../../../../public/og-bg.png'),
      loadAsset('./fonts/Inter-Regular.ttf'),
      loadAsset('./fonts/BeVietnamPro-Medium.ttf'),
    ]);

    const bgImageBase64 = `data:image/png;base64,${arrayBufferToBase64(bgImageBuffer)}`;
      ...
      return new ImageResponse(
        (
          ...
          >
            ...
            >
              <BoredBrainLogo width={120} height={120} color="#ffffff" />
              <div
                style={{
                  fontSize: 56,
                  color: '#ffffff',
          ...
          >
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <BoredBrainLogo width={28} height={28} color="#ffffff" />
