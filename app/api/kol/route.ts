...
const kolTool = tool({
    description: 'Search X posts from key opinion leaders (KOLs) for recent information and discussions with the ability to filter by X handles, date range, and post engagement metrics.',
    ...
    async execute({
        ...
        const result = await generateText({
            model: boredbrain.languageModel('boredbrain-grok-4-fast'),
            prompt: query,
            maxOutputTokens: 10,
            providerOptions: {
                xai: {
...
export type KOLMessage = UIMessage<never, UIDataTypes, InferUITools<typeof tools>>;
...
export async function POST(req: Request) {
    ...
    const result = streamText({
        ...
        // Force a single kol tool call
        toolChoice: { type: 'tool', toolName: 'kol' },
        activeTools: ['kol'],
        providerOptions: {
            groq: {
        ...
        system: `You are a helpful assistant that searches for X posts from key opinion leaders (KOLs) and influential accounts. You will be given a search query and you will need to search for the posts and return the results in a structured format.

        Today's date is ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: '2-digit', weekday: 'short' })}.
        The date range is from 15 days ago to today unless the user specifies otherwise.

        The tool to use is kol.
        ...
        The tool name is kol - use it to search X/Twitter posts from influential accounts and key opinion leaders.
        `,
        tools: {
            kol: kolTool,
        } as ToolSet,
