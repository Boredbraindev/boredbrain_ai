  ...
  return (
    <Card
      className={`shadow-none border border-primary/50 cursor-pointer relative overflow-hidden hover:border-primary/40 ${
        signal.status === 'running' ? 'border-primary/30' : ''
      } ${signal.status === 'archived' ? 'opacity-75' : ''}`}
      onClick={handleCardClick}
    >
      {/* Border trail for running signals */}
        ...
        <div className="space-y-1">
          ...
          {signal.nextRunAt && signal.status === 'active' && (
            <p className="text-xs text-muted-foreground">
              Next Run: {formatNextRun(signal.nextRunAt, signal.timezone)}
            </p>
          )}
          ...
          {signal.lastRunAt && (
            <div className="flex items-center gap-2">
              <p className="text-xs text-muted-foreground">
                Last Run: {formatNextRun(signal.lastRunAt, signal.timezone)}
              </p>
              {signal.lastRunChatId && (
                <Link
                  href={`/search/${signal.lastRunChatId}`}
          ...
          {!signal.lastRunAt && signal.frequency === 'once' && signal.status === 'paused' && (
