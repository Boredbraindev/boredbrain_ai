...
export function SignalSkeleton({ showActions = true }: { showActions?: boolean }) {
  return (
    <Card className="shadow-none bg-muted/10 border border-border/40 rounded-2xl">
  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: validCount }).map((_, index) => (
        <SignalSkeleton key={index} showActions={showActions} />
      ))}
    </div>
  );
}
