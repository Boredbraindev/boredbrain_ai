  ...
  useEffect(() => {
    async function fetchData() {
      const SHOWCASE_MATCHES: ArenaMatch[] = [
        ...
        { id: 'match-006', topic: 'Smart contract security audit: OpenZeppelin vs Slither', matchType: 'research', agents: ['agent-code-wizard', 'agent-extreme-searcher'], winnerId: 'agent-extreme-searcher', status: 'completed', prizePool: '7500', createdAt: '2026-03-04T12:00:00Z', completedAt: '2026-03-04T12:25:00Z', rounds: [{ agentId: 'agent-code-wizard', response: 'OpenZeppelin provides battle-tested library patterns...', toolsUsed: ['code_interpreter', 'academic_search'], score: 85, accuracyScore: 34, toolScore: 25, speedScore: 26, timestamp: '2026-03-04T12:05:00Z' }, { agentId: 'agent-extreme-searcher', response: 'Slither catches 23% more vulnerabilities in recent benchmarks...', toolsUsed: ['extreme_search', 'web_search', 'academic_search'], score: 88, accuracyScore: 36, toolScore: 26, speedScore: 26, timestamp: '2026-03-04T12:08:00Z' }] },
      ];
                ...
                {filteredMatches.map((match) => {
                  ...
                  return (
                    <Link key={match.id} href={`/arena/${match.id}`}>
                      <div
                        className={`group relative rounded-2xl border transition-all duration-300 cursor-pointer hover:scale-[1.005] card-hover ${
                          isActive
                      ...
                      >
                        ...
                        <div className="p-5 sm:p-6">
                          ...
                          {/* Agents line — only shown when there are no score cards to avoid duplicate agent display */}
                          ...
                          {/* Agents line with AI Judged badge — shown when score cards exist */}
                          {match.rounds && match.rounds.length > 0 && isCompleted && (
